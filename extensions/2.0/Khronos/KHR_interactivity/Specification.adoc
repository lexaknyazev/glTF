...
If the input reference value is null or it is not contained in the dynamic array of delay activation references, the `pointer/get` operation fails and thus sets the `isValid` output value is false and the `value` output value is null.
