...
| Output value sockets
| `int lastDelayIndex` | The delay index assigned during the last successful interactivity node's execution
...
The internal state of this operation consists of an integer `lastDelayIndex` value initialized to -1 and a dynamic array of activation indices scheduled by the interactivity node executions. This array is initially empty and its maximum size is implementation-specific.

The internal state of an execution graph having one or more interactivity nodes with the `flow/setDelay` operation includes a dynamic array of activation indices scheduled from all such interactivity nodes. This array is initially empty and its maximum size is implementation-specific.
...
.. activate the `err` output flow and skip the next steps.
6. Set `lastDelayIndex` to a non-negative value representing the delayed flow activation being scheduled. This value **MUST** be unique across all previous activations of all interactivity nodes with the `flow/setDelay` operation.
7. Push the value of `lastDelayIndex` to the graph and interactivity node arrays of activation indices.
...
2. For each activation index value in the interactivity node's array of activation indices:
.. Remove this activation index value from the interactivity node's and the graph's arrays of activation indices.
...
.Examples
====
- Let the `pointer` configuration value be `"/nodes/{N}/weights/{W}"`. Then the operations using this template pointer string have the `N` and `W` input value sockets. Let the runtime `N` value be 1 and the runtime `W` value be 2. Then the effective JSON Pointer is `"/nodes/1/weights/2"`.

- Let the `pointer` configuration value be `"/nodes/{index}/extras/{{index}}"`. Then the operations using this template pointer string have the `index` input value socket. Let the runtime `index` value be 2. Then the effective JSON Pointer is `"/nodes/2/extras/{index}"`.
====
...
- If the `pointer` configuration value is `"/nodes/{myId}/scale"`, the operation has the `myId` input value socket, which value denotes the glTF node index.
====
...
If the `pointer` configuration value is not provided, if it is not a string, if it is invalid (as defined in the previous sections), or if it contains a template parameter `{value}`, the interactivity node is invalid and the graph **MUST** be rejected.
...
If the `pointer` configuration value is not provided, if it is not a string, if it is invalid (as defined in the previous sections), or if it contains template parameters `{value}`, `{duration}`, `{p1}`, or `{p2}` the interactivity node is invalid and the graph **MUST** be rejected.
...
If multiple interactivity nodes with this operation exist in the graph, they **MUST** be activated sequentially in the order they appear in JSON.
...
Interactivity nodes with this operations are activated when a tick occurs. There will be at most one tick per rendered frame, which **SHOULD** align with frame time, but there are no guarantees of time elapsed between ticks.

The internal state of this operation consists of two floating-point time values initialized to NaN. They **MUST** be set to their effective values before the `out` output flow is activated.
