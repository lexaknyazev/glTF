...
.2+| Input value sockets
d| `ref a` | First argument
d| `ref b` | Second argument
| Output value sockets
| `bool value` | True if the input arguments are equal; false otherwise
|===

Valid references are equal if and only if they refer to the same object.

If any argument is an invalidated reference, the comparison result is undefined.

All null references are equal to each other. A null reference is not equal to a non-null reference.
